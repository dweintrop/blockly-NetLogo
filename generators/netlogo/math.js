...
Blockly.NetLogo.addReservedWords('math,random');
...
Blockly.NetLogo['math_single'] = function(block) {
  ...
  Blockly.NetLogo.definitions_['import_math'] = 'import math';
...
Blockly.NetLogo['math_constant'] = function(block) {
  ...
  var CONSTANTS = {
    'PI': ['math.pi', Blockly.NetLogo.ORDER_MEMBER],
    'E': ['math.e', Blockly.NetLogo.ORDER_MEMBER],
    'GOLDEN_RATIO': ['(1 + math.sqrt(5)) / 2', Blockly.NetLogo.ORDER_MULTIPLICATIVE],
    'SQRT2': ['math.sqrt(2)', Blockly.NetLogo.ORDER_MEMBER],
    'SQRT1_2': ['math.sqrt(1.0 / 2)', Blockly.NetLogo.ORDER_MEMBER],
    'INFINITY': ['float(\'inf\')', Blockly.NetLogo.ORDER_ATOMIC]
  };
...
Blockly.NetLogo['math_number_property'] = function(block) {
  ...
  if (dropdown_property == 'PRIME') {
    ...
    var functionName = Blockly.NetLogo.provideFunction_(
        'math_isPrime',
        ['def ' + Blockly.NetLogo.FUNCTION_NAME_PLACEHOLDER_ + '(n):',
  ...
  switch (dropdown_property) {
    ...
    case 'NEGATIVE':
      ...
      break;
    case 'DIVISIBLE_BY':
      var divisor = Blockly.NetLogo.valueToCode(block, 'DIVISOR',
          Blockly.NetLogo.ORDER_MULTIPLICATIVE);
      // If 'divisor' is some code that evals to 0, Python will raise an error.
      if (!divisor || divisor == '0') {
        return ['False', Blockly.NetLogo.ORDER_ATOMIC];
      }
...
Blockly.NetLogo['math_change'] = function(block) {
  // Add to a variable in place.
  var argument0 = Blockly.NetLogo.valueToCode(block, 'DELTA',
      Blockly.NetLogo.ORDER_ADDITIVE) || '0';
  var varName = Blockly.NetLogo.variableDB_.getName(block.getFieldValue('VAR'),
  ...
  switch (func) {
    ...
    case 'MODE':
      var functionName = Blockly.NetLogo.provideFunction_(
          ...
          ['def ' + Blockly.NetLogo.FUNCTION_NAME_PLACEHOLDER_ + '(some_list):',
           '  modes = []',
    ...
    case 'STD_DEV':
      ...
      var functionName = Blockly.NetLogo.provideFunction_(
          ...
          ['def ' + Blockly.NetLogo.FUNCTION_NAME_PLACEHOLDER_ + '(numbers):',
           '  n = len(numbers)',
           '  if n == 0: return',
...
Blockly.NetLogo['math_constrain'] = function(block) {
  // Constrain a number between two limits.
  var argument0 = Blockly.NetLogo.valueToCode(block, 'VALUE',
      Blockly.NetLogo.ORDER_NONE) || '0';
  var argument1 = Blockly.NetLogo.valueToCode(block, 'LOW',
      Blockly.NetLogo.ORDER_NONE) || '0';
  var argument2 = Blockly.NetLogo.valueToCode(block, 'HIGH',
      Blockly.NetLogo.ORDER_NONE) || 'float(\'inf\')';
  var code = 'min(max(' + argument0 + ', ' + argument1 + '), ' +
      argument2 + ')';
  return [code, Blockly.NetLogo.ORDER_FUNCTION_CALL];
};

Blockly.NetLogo['math_random_int'] = function(block) {
  // Random integer between [X] and [Y].
  Blockly.NetLogo.definitions_['import_random'] = 'import random';
  var argument0 = Blockly.NetLogo.valueToCode(block, 'FROM',
      Blockly.NetLogo.ORDER_NONE) || '0';
  var argument1 = Blockly.NetLogo.valueToCode(block, 'TO',
      Blockly.NetLogo.ORDER_NONE) || '0';
  var code = 'random.randint(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.NetLogo.ORDER_FUNCTION_CALL];
};

Blockly.NetLogo['math_random_float'] = function(block) {
  // Random fraction between 0 and 1.
  Blockly.NetLogo.definitions_['import_random'] = 'import random';
  return ['random.random()', Blockly.NetLogo.ORDER_FUNCTION_CALL];
};
